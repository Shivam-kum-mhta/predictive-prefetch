...
# Add the parent directory to the path to import markov module
...
def load_test_data():
    ...
    # Load filtered behaviors data
    print("\nLoading MINDsmall_dev_filtered_behaviors.tsv...")
    behaviours_df = pd.read_csv('../../../dataset/train_behaviours.tsv', sep='\t',
                               names=['Serial', 'UserID', 'Time', 'History', 'Impressions'])
    
    print(f"  Test behaviors: {len(behaviours_df):,}")
    
    # Load news data for context (both train and dev)
    print("\nLoading news data for context...")
    news_train_df = pd.read_csv('../../../dataset/MINDsmall_train_news.tsv', sep='\t',
                               names=['NewsID', 'Category', 'Author', 'Title', 'Abstract', 
                                      'URL', 'TitleEntities', 'AbstractEntities'])
    news_dev_df = pd.read_csv('../../../dataset/MINDsmall_dev_news.tsv', sep='\t',
                             names=['NewsID', 'Category', 'Author', 'Title', 'Abstract', 
                                    'URL', 'TitleEntities', 'AbstractEntities'])
    
    # Combine news dataframes
    all_news_df = pd.concat([news_train_df, news_dev_df], ignore_index=True)
    all_news_df = all_news_df.drop_duplicates(subset=['NewsID'])
    
    print(f"  Training news articles: {len(news_train_df):,}")
    print(f"  Dev news articles: {len(news_dev_df):,}")
    print(f"  Total unique news articles: {len(all_news_df):,}")
    
    return behaviours_df, all_news_df

def load_trained_model(model_path='markov_model.pkl'):
...
def evaluate_model(mc, test_cases, top_k_values=[1, 5, 10, 20, 50]):
    """
...
def main():
    ...
    try:
        ...
        mc = load_trained_model('markov_model.pkl')
        ...
        results = evaluate_model(mc, test_cases, top_k_values=[1, 5, 10, 20, 50])
